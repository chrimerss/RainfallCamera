...
from model import BGDescriminator
...
def train():
	...
	#tensorboard setup
	writer= SummaryWriter('logs/BackGroundDiscriminator')

	for epoch in range(epoches):
		print("Epoch %d/%d"%(epoch, epoches) )
		print('-'*20)
		a= list(model.parameters())[3].clone()
	...
	torch.save(model.state_dict(), 'logs/BGDiscriminator.pth')
