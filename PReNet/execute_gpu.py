...
class RRCal(object):
	...
	def pretrained_model(self, model_path="PReNet1.pth"):
		...
		if opt.use_GPU:
			model = model.cuda()
		model.load_state_dict(torch.load(os.path.join(opt.logdir, model_path)))
		model.eval()
...
if __name__=='__main__':
	rate_cal= RRCal('D:\\Radar Projects\\lizhi\\CCTV\\Rain Detection\\CSC\\MS-CSC-Rain-Streak-Removal')
	...
	df= rate_cal.event_based_im('20181208')
	np.save(opt.save_path+'\\'+'20181208', df)
	df.to_excel(opt.save_path+'\\'+'20181208'+'.xlsx')
